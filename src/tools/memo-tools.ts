...
import { Visibility, MemoState } from '../types.js';
...
export const listMemosSchema = z.object({
  ...
  orderBy: z
    ...
    .describe(
      'Sort order (e.g., "pinned desc, display_time desc"). Supports: pinned, display_time, create_time, update_time, name'
    ),
  filter: z
    .string()
    .optional()
...
export function createMemoTools(client: MemosClient) {
  return {
    ...
    list_memos: {
      ...
      handler: async (input: ListMemosInput) => {
    ...
    get_memo: {
      ...
      handler: async (input: GetMemoInput) => {
    ...
    update_memo: {
      ...
      handler: async (input: UpdateMemoInput) => {
    ...
    delete_memo: {
      ...
      handler: async (input: DeleteMemoInput) => {
